...
- `EXPLAIN`: 옵티마이저가 세운 계획과 예상 비용(cost)만 보여준다. 쿼리를 실제로 실행하지
  않는다.
- `EXPLAIN ANALYZE`: 쿼리를 **실제로 실행**하면서 각 노드의 실제 소요 시간(`actual
  time`)과 실제 처리한 행 수(`actual rows`)를 함께 보여준다. 옵티마이저의 "예상"과
  "실제"를 비교할 수 있는 게 핵심 가치.
  - 주의: `INSERT/UPDATE/DELETE`에 `EXPLAIN ANALYZE`를 쓰면 실제로 데이터가 바뀐다.
    `BEGIN; EXPLAIN ANALYZE ...; ROLLBACK;`로 감싸는 습관이 안전하다(SELECT는 무관).

### Seq Scan vs Index Scan
- **Seq Scan(순차 스캔)**: 테이블의 모든 페이지를 처음부터 끝까지 읽으며 조건에 맞는
  행을 찾는다. 조건에 맞는 행의 비율이 높으면(선택도가 낮으면) 오히려 이게 더 싸다.
- **Index Scan**: 인덱스(B-tree 등)를 먼저 탐색해서 조건에 맞는 행의 위치를 찾고, 그
  위치로 힙(테이블 본체)에 접근한다. 조건에 맞는 행의 비율이 낮을 때(선택도가 높을 때)
  유리하다.
- **Index Only Scan**: 필요한 컬럼이 전부 인덱스 안에 있으면 힙 접근 없이 인덱스만으로
  끝낸다(챕터 10 커버링 인덱스에서 다룸).
- 핵심: 옵티마이저는 **비용(cost) 기반**으로 플랜을 선택한다 — "인덱스가 있으면 무조건
  쓴다"는 가정은 틀렸다. 이건 이번 문서 전체를 관통하는 주제.
