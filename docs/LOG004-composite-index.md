...
복합 인덱스 `(A, B)`는 B-tree 안에서 먼저 A로 정렬하고, 같은 A 값 안에서 다시 B로
정렬한 구조다. 그래서:
- `WHERE A = ?` → 인덱스 사용 가능 (A만으로 좁혀짐)
- `WHERE A = ? AND B = ?` → 인덱스 사용 가능 (A로 좁힌 뒤 그 안에서 B도 좁힘)
- `WHERE B = ?` (A 조건 없이) → 인덱스를 거의 못 씀. 전화번호부를 "성"으로
  정렬해놓고 "이름"만으로 찾으려는 것과 같아서, 결국 전체를 훑어야 함.

즉 `(A, B)`와 `(B, A)`는 완전히 다른 용도의 인덱스다 — 컬럼 순서를 바꾸면 지원하는
쿼리 패턴이 바뀐다.

### 정렬(`ORDER BY`)도 인덱스로 해결할 수 있다
B-tree는 이미 정렬된 구조라, 조건뿐 아니라 `ORDER BY`도 인덱스로 해결할 수 있다 —
조건에 맞는 시작 위치에서부터 순서대로 읽기만 하면 되므로 별도의 `Sort` 실행 단계가
필요 없어진다. 이번 챕터의 핵심 실습 대상.
